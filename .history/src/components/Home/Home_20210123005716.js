...
const Home = () => {
    ...
    const handleChange = async (e) =>{
        if(e.target.name==='currency'){
              if(e.target.value){
                try {
                    const response = await axios.get(`https://restcountries.eu/rest/v2/currency/${e.target.value}
                    `);
        ...
        <Container className='py-5'>
           <Row>
               <Col md={10}>
                   <Row>
                      {
                        filterCountries.length ===0 ?<h2>Not found</h2>:filterCountries.map((country,index)=> <Countries
                        key={index} 
                        country={country}/>)
                      }
                   </Row>
